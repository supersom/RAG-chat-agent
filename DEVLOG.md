...
## 2026-07-25 04:01 PDT — Keyword search results silently dropped when one document dominates the top matches

**Context:** With the sync fully working and the corpus indexed (grew to 24,440 chunks over the course of the night), live-tested "What is 1406.2294 about?" against the deployed chat. It answered from the vector-only fallback ("I don't have information about the specific identifier..."), even though the paper (`A Fast, Minimal Memory, Consistent Hash Algorithm (1406.2294).pdf`) was correctly indexed - confirmed by replaying the exact production query directly against a fresh download of the live `.sqlite` (`node` + `better-sqlite3`), which returned the correct document as the clear top match (bm25 rank -11.28, next-best unrelated match -7.6).

**Root cause investigation:** Added temporary diagnostic logging to `retrieveContext` and `searchKeywordIndex` (`9c70d5c`, later removed), deployed, and re-ran the same live query. This proved `getKbDataSource` resolved correctly and `searchKeywordIndex` returned 3 rows - but the final merged `ragSources` sent to the LLM contained zero keyword-type results, all three vector-only. The bug was in `mergeSources` (`app/lib/rag.ts`): it dedupes keyword hits by document (`s3Uri`), but the merge loop unconditionally overwrote `keywordRank` on every duplicate match instead of keeping the best. When the top 3 keyword hits are all chunks from the *same* dominant document - exactly what happens for a narrow/identifier query - the rank degrades from 1 to the last (worst) chunk's index each iteration, RRF-tying it with the weakest vector match, and losing the tie-break (vector entries are inserted into the merge map first, so they win ties in the stable sort). A related bug in the same block unconditionally stamped `retrievalType: "vector"` on any existing entry a keyword hit merged into, even when that entry had no vector match at all.

**Decision:** Fixed by only setting `keywordRank` the first time (`if (existing.keywordRank === undefined)`), and only overwriting `retrievalType` to `"vector"` when the entry actually has a `vectorRank`. TDD'd first (`app/lib/rag.test.ts`, exported `mergeSources` for testability): both new tests failed on the original code exactly as predicted, passed after the fix. Deployed (`06fea13`, Amplify job 22) and live-verified: `Parsed RAG Sources` for the same query now includes `keyword:40800001` (the correct paper, `retrievalType: 'keyword'`, RRF-tied with the top vector match) at position 2 of 3.
